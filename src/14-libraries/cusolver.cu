...
int main(void)
{
    ...
    for (int n = 0; n < N2; ++n) //x for real and y for complex
    {
        A_cpu[0].x = 0;
        A_cpu[1].x = 0;
        A_cpu[2].x = 0;
    ...
    CHECK(cudaMemcpy(A, A_cpu, sizeof(cuDoubleComplex) * N2, 
        cudaMemcpyHostToDevice));//transfer A_cpu from host to device A
    ...
    cusolverEigMode_t jobz = CUSOLVER_EIG_MODE_NOVECTOR;//just cal eigenvalue
