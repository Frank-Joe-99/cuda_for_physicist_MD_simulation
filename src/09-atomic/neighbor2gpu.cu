...
void __global__ find_neighbor_atomic//use atomic function
(
    int *d_NN, int *d_NL, const real *d_x, const real *d_y,
    const int N, const real cutoff_square
)// transform all kinds of data from host to device 
{
    ...
    if (n1 < N)
    {
        ...
        for (int n2 = n1 + 1; n2 < N; ++n2)
        {
            ...
            }// use atomicadd to keep d_NN correct
        }
    }
}

void __global__ find_neighbor_no_atomic//do not use atomic function
(
    int *d_NN, int *d_NL, const real *d_x, const real *d_y,
    const int N, const real cutoff_square
)
